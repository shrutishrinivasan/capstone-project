...
from ragas.llms.base import LangchainLLMWrapper  # For wrapping the LLM
...
async def evaluate_faithfulness(response, context, evaluator_llm):
    try:
        sample = SingleTurnSample(  user_input="",  # Not used by Faithfulness metric
                                    response=response, retrieved_contexts=[context] if context else [] )
...
# Context Precision and Context Recall evaluation functions
...
#============= METRIC 7: CONTEXT PRECISION WITH REFERENCES ========================================
...
async def evaluate_non_llm_context_recall(context, reference_contexts):
    try:
...
async def evaluate_finmentor():
    ...
    persist_dir = "./chroma_fin_mentor_ollama_eval"
    ...
    for item in tqdm(test_data, desc="Evaluating questions"):
        ...
        context_precision_wo_ref = await evaluate_context_precision_without_reference(question, response, context, wrapped_llm)
        context_precision_w_ref = await evaluate_context_precision_with_reference(question, response, reference, context, wrapped_llm)
        context_recall = await evaluate_context_recall(question, response, reference, context, wrapped_llm)
        non_llm_context_recall = await evaluate_non_llm_context_recall(context, reference_contexts)
    ...
    with open("finmentor_ollama_ragas_evaluation2.json", "w") as f:
    ...
    print("Detailed results saved to finmentor_ollama_ragas_evaluation2.json")
