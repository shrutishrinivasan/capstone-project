...
'''
# Initialize Ollama LLM with Mistral (shared between both bots)
model = OllamaLLM(
    model="mistral", 
...
def initialize_database():
    ...
    if count == 0 and os.path.exists("data\syn_daily_exp_v3.csv"):
        # Import data from CSV if table is empty
        df = pd.read_csv("data\syn_daily_exp_v3.csv")
        ...
        mycon.commit()
        print(f"Imported {len(records)} records from syn_daily_exp_v3.csv")
...
def data_digger():
    ...
    # Chat input
    if prompt := st.chat_input("Ask questions about your transaction history..."):
        # Add user message to chat history
        st.session_state.digger_messages.append({"role": "user", "content": prompt})
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Generating SQL query..."):
                ...
                sql_query = get_mistral_response(prompt, data_digger_prompt_template)
        ...
        st.session_state.digger_messages.append({"role": "assistant", "content": formatted_response})
...
def fin_mentor():
    ...
    # Chat input
    if prompt := st.chat_input("What is your finance question?"):
...
def initialize_database():
    ...
    if count == 0 and os.path.exists("data\syn_daily_exp_v3.csv"):
        ...
        cursor.executemany(sql_query, records)
        mycon.commit()
        print(f"Imported {len(records)} records from data\syn_daily_exp_v3.csv")
...
def data_digger():
            ...
            try:
                ...
                if isinstance(result, list) and result and isinstance(result[0], str) and result[0].startswith("Error:"):
                    ...
                    if columns:
                        ...
                        df = pd.DataFrame(result, columns=columns)
                        formatted_response += "Here's what I found:\n\n"
                        # The actual dataframe will be displayed in the chat_container
                        st.session_state.digger_df = df
                    else:
                        formatted_response += "No results found."
    ...
    with chat_container:
        for message in st.session_state.digger_messages:
            with st.chat_message(message["role"]):
                st.markdown(message["content"])
                # If this is an assistant message with a dataframe result, display it
                if message["role"] == "assistant" and "digger_df" in st.session_state:
                    if "Here's what I found:" in message["content"]:
                        st.dataframe(st.session_state.digger_df)
                        # Clear the dataframe from session state to avoid showing it again
                        if st.session_state.digger_messages[-1] == message:
                            # Only clear if this is the last message
                            st.session_state.digger_df = None
